...
"""layer {
  ...
  prior_box_param {
    ...
    def mbox_conf(self, bottom, inp, num):
       name_1 = bottom + "_mobox_conf_3*3"
       self.conv(name_1, inp, 3, bias=True, bottom=bottom, group=inp)
       name_2 = bottom + "_mbox_conf"
       self.conv(name_2, num, 1, bias=True, bottom=name_1)
       self.permute(name_2)
       self.flatten(name_2)
    def mbox_loc(self, bottom, inp, num):
       name_1 = bottom + '_mbox_loc_3*3'
       self.conv(name_1, inp, 3, bias=True, bottom=bottom, group=inp)
       name_2 = bottom + "_mbox_loc"
       self.conv(name_2, num, 1, bias=True, bottom=name_1)
       self.permute(name_2)
       self.flatten(name_2)
    ...
    def mbox(self, bottom, inp, num):
       ...
       if self.first_prior:
           self.mbox_prior(bottom, min_size, max_size, ratios)
           self.first_prior = False
       else:
           self.mbox_prior(bottom, min_size, max_size,ratios)
       self.anchors.pop(0) # pop anchor scale
...
"""layer {
    ...
    def generate(self, stage, gen_ssd, size, class_num):
      ...
      if stage == "train":
          ...
          self.data_deploy()
      self.conv_bn_relu_with_factor("conv1", 7, 3, 2) #224/112
      self.conv_dw_pw("conv2", 7,  16, 1) # 112/112
      self.conv_dw_pw("conv3", 16, 16, 2) # 112/56
      self.conv_dw_pw("conv4", 16, 16, 1) # 56/56
      self.conv_dw_pw("conv5", 16, 16, 2) # 56/28
      self.conv_dw_pw("conv6", 16, 16, 1) # 28/28
      self.conv_dw_pw("conv7", 16, 32, 2) # 28/14
      self.conv_dw_pw("conv8", 32, 32, 1) # 14/14
      self.conv_dw_pw("conv9", 32, 32, 1) # 14/14
      self.conv_dw_pw("conv10", 32, 32, 1) # 14/14
      self.conv_dw_pw("conv11",32, 32, 1) # 14/14
      self.conv_dw_pw("conv12",32, 64, 1) # 14/14
      if gen_ssd is True:
          self.mbox("conv6", 16, 4)
          self.mbox("conv10", 32, 4)
          self.mbox("conv12", 64, 4)
          self.concat_boxes(['conv6', 'conv10', 'conv12'])
          if stage == "train":
             self.ssd_loss()
          elif stage == "deploy":
             self.ssd_predict()
...
def create_ssd_anchors(num_layers=3,
  ...
  scale_1 = [0.1,0.2,0.7]
  step = [0.14142136, 0.37416574, 0.916515]
  # return zip(scales[:-1], scales[1:])
  return zip(scale_1, step)
